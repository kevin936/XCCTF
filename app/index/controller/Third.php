...
class Third extends Frontend
{
    ...
    public function prepare()
    {
        ...
        if ($this->auth->id) {
            $this->redirect(url("index/third/bind")->build() . "?" . http_build_query(['platform' => $platform, 'url' => $url]));
        }
        ...
        Lang::load([
            app_path() . 'index' . DS . 'lang' . DS . Lang::getLangSet() . DS . 'user' . EXT,
        ]);

        $this->view->assign('userinfo', $userinfo['userinfo']);
        $this->view->assign('platform', $platform);
        $this->view->assign('url', $url);
        $this->view->assign('bindurl', url("index/third/bind")->build() . '?' . http_build_query(['platform' => $platform, 'url' => $url]));
